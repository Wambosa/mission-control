// Consume-once registry of starting prompts for sessions created with one,
// keyed by taskId. createSession stashes the prompt here; TerminalPane reads it
// at the first seedable spawn and passes it as the PTY's initialInput, then
// it's gone — so reloads and re-spawns never re-inject the prompt. Keeping it
// out-of-band means the normal session path is completely unaffected.
